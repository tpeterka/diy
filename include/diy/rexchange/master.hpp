  ...
  class Master
  {
    ...
    public:
      ...
      */
      ...
      //! exchange the queues between possibly remote blocks (not in the link) (collective operation)
      inline void   rexchange();
    ...
    public:
      ...
      inline void       rflush();           // makes sure all the serialized queues migrate to their target processors
...
rexchange()
{
  auto scoped = prof.scoped("rexchange");
  execute();

  log->debug("Starting rexchange");
...
rcomm_exchange(ToSendList& to_send, int out_queues_limit)
{
  ...
  // issend outgoing queues, up to the out_queues_limit
  while(inflight_sends_.size() < (size_t)out_queues_limit && !to_send.empty())
  {
    int from = to_send.front();
    ...
    for (OutgoingQueues::iterator it = outgoing.begin(); it != outgoing.end(); ++it)
    {
      ...
      log->debug("rexchange: Processing queue:      {} <- {} of size {}", to, from, outgoing_[from].queues[to_proc].size());

      // There may be local outgoing queues that remained in memory
      if (proc == comm_.rank())     // sending to ourselves: simply swap buffers
      {
        log->debug("rexchange: Moving queue in-place: {} <- {}", to, from);

        QueueRecord& in_qr  = current_incoming.map[to].records[from];
        bool in_external  = block(lid(to)) == 0;
        if (in_external)
        {
          log->debug("rexchange: Unloading outgoing directly as incoming: {} <- {}", to, from);
          MemoryBuffer& bb = it->second;
          in_qr.size = bb.size();
          if (queue_policy_->unload_incoming(*this, from, to, in_qr.size))
            in_qr.external = storage_->put(bb);
        ...
        {
          log->debug("rexchange: Swapping in memory:    {} <- {}", to, from);
          MemoryBuffer& bb = current_incoming.map[to].queues[from];
          bb.swap(it->second);
          bb.reset();
          in_qr.size = bb.size();
  ...
  while (!done)
  {
      ...
      while (ostatus)
      {
          ...
          if (ir.info.from == -1) // uninitialized
          {
              log->debug("rexchange: uninitialized recv:    {} <- {}", ir.info.to, ir.info.from);
              MemoryBuffer bb;
          ...
          {
              log->debug("rexchange: initialized recv:    {} <- {}", ir.info.to, ir.info.from);
              size_t start_idx = ir.message.buffer.size();
              size_t count = ostatus->count<char>();
              ir.message.buffer.resize(start_idx + count);
          ...
          if (unload_queue)
          {
              log->debug("rexchange: Directly unloading queue {} <- {}", to, from);
              external = storage_->put(ir.message); // unload directly
          }
          else
          {
              log->debug("rexchange: Unloading queue {} <- {} size {}", to, from, size);
              in->map[to].queues[from].swap(ir.message);
              in->map[to].queues[from].reset();     // buffer position = 0
          }
          in->map[to].records[from] = QueueRecord(size, external);
      ...
      else
      {
          ...
          for (it = inflight_sends_.begin(); it != inflight_sends_.end(); it++)
              if (!it->request.test())
                  break;
          if (it == inflight_sends_.end())          // if all sends completed, enter ibarrier
          {
...
rflush()
{
  ...
  log->debug("rflush: to_send.size(): {}", to_send.size());
  ...
  do
  {
    ...
    if (cur - start > wait*1000)
    {
        log->warn("rflush: Waiting in flush [{}]: {} - {} out of {}",
                  comm_.rank(), inflight_sends_.size(), incoming_[exchange_round_].received, expected_);
        wait *= 2;
    }
#endif
  ...
  log->debug("rflush: Done in flush");
  //show_incoming_records();
